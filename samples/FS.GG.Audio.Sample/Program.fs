...
// so 3D voices spatialize (a plain IAudioBackend would degrade them to non-positional).
type private ConsoleBackend() =
    interface IMixingBackend with
        ...
        member _.PlayAt(sound, gain, pan) =
            let (SoundId s) = sound
            printfn "        > play %-5s gain=%.3f pan=%+.2f (spatial)" s gain pan
...
let main _ =
    ...
    // Fire a positional sfx to the right (x=+3), and duck the music under it for 1s.
